...
Для того, чтобы присоединить UART-переходник к плате камеры, нужно воспользоваться проводами с клеммами. Будьте внимательны при соединении, не повредите элементы схемы и не замкните контакты между собой!
...
Для начала загрузите двоичный файл загрузчика по [ссылке](https://github.com/OpenIPC/firmware/releases/download/latest/u-boot-t31n-nor.bin). У вас должен получиться бинарный файл **u-boot-t31n-nor.bin**.

Поместите полученный файл на свою SD карту. 

**ВНИМАНИЕ!** _Если вы используете ту же самую карту памяти, что и в прошлом пункте, предварительно отформатируйте ее в Файловой системе Fat32 c таблицей MBR (MS-DOS). Не используйте таблицу GPT! Если вы используете ОС Windows - это самое обычное форматирование. Просто подключите карту памяти и Windiows сама предложит ее отформатировать._
_________________
...
fatls mmc 0:1
```
Должны выйти данные вашей карты памяти. Если вышли какие-либо ошибки, не продолжайте до тех пор, пока они не будут устранены! В противном случае перепрошивка камеры будет возможна только на специальном оборудовании.
...
```
_Заводской загрузчик не поддерживает сохранение переменных, поэтому если камера была перезагружена, вводить придется по-новой._
...
Скомандуйте `reset` в консоли загрузчика, камера перезагрузится. Теперь загрузку камеры можно прерывать нажатием Enter
________________
...
В папке [https://github.com/OpenIPC/builder/releases](https://github.com/OpenIPC/builder/releases) вы найдете самые свежие релизы OpenIPC для своего устройства.

![изображение](https://github.com/user-attachments/assets/05c63b5b-ec31-42dc-834c-59c49d3f4f9e)

Вам потребуется архив  **t31_lite_xiaomi-mjsxj03hl-jxq03-nor.tgz**
_Обратите внимание, что разные камеры одной модели могут комплектоваться различными сенсорами.  Если в вашей камере установлен сенсор, отличающийся от jxq03, то скачайте файл **t31_lite_xiaomi-mjsxj03hl-nor.tgz**_

В архиве вы найдете 4 файла: образ корневой ФС и ядро, а также контрольные суммы к ним. Разархивируйте их на вашу карту памяти. Туда же добавьте файл [скрипта](https://github.com/OpenIPC/device-mjsxj03hl/blob/master/boot.scr) для установки. Поместите карту памяти с файлами в слот камеры. Подключите UART, запустите программу-терминал и подайте питание на камеру. Вы сможете видеть, как произойдет прошивка Вашей камеры в автоматическом режиме.

![изображение](https://github.com/user-attachments/assets/355d9bb1-4dd3-4b5b-855b-7f6bee264e99)

Далее:
1) Отключите питание
2) Извлеките карту памяти. _Следует удалить файлы скрипта и прошивки с нее._
3) Подайте питание. Камера в автоматическом режиме выполнит все необходимые операции и загрузит ОС. 
...
openipc-t31 login: 
```
Введите логин `root` , пароль по умолчанию `12345`
...
> [!IMPORTANT]
> Не забудьте удалить файлы прошивки и скрипт с Вашей карты памяти!
______________
## Настройка
